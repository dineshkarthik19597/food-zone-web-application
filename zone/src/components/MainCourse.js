  ...
  useEffect(() => {
    axios
      .get('http://localhost:5000/api/foods/type?', {
        params: { item_type: 'Main Course' }
      })
      .then(res => {
        setMainItems(res.data.items || []);
      })
      .catch(() => {
        setMainItems([]);
      });
      ...
      {items.length > 0 ? (
        items.map((item, index) => (
          <div className="card" key={index}>
             <img src={item.image.startsWith('images/') ? item.image : `http://localhost:5000/${item.image}`} alt={item.item_name} className="card-img" />
            <h5>{item.item_name}</h5>
            <button className="snack-btn" onClick={() => handleCart(item)}>Add to cart</button>
          </div>
        ))
      ) : (
        <p className="no-data">No main course items available.</p>
      )}
    </div>
  );
